...
const rawIncidentsService = {
  ...
  async getIncident(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v0/tenants/me/incidents` + (params ? `?${new URLSearchParams(params).toString()}` : '');
  ...
  async getIncident(data?: any, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v0/tenants/me/incidents`;
  ...
  async getIncident(incidentId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v0/tenants/me/incidents/${incidentId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getHaltStudy(incidentId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/incidents/${incidentId}/halt-study`;
