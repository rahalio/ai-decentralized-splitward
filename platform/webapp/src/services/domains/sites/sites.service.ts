...
const rawSitesService = {
  /**
   * List Sites
   */
  async getSite(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/sites` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Create Site
   */
  async createSite(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/sites`;
   ...
   * Get Site
   */
  async getSite(siteId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/sites/${siteId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const sitesService = makeService(rawSitesService, "sites");
