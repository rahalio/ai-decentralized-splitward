 ...
 * AuditPacks Service
 *
 * API client for audit-packs domain.
...
const rawAuditPacksService = {
  /**
   * List AuditPacks
   */
  async getAuditPack(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/audit-packs` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Create AuditPack
   */
  async getAuditPack(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/audit-packs`;
   ...
   * Get AuditPack
   */
  async getAuditPack(packId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/audit-packs/${packId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const audit-packsService = makeService(rawAuditPacksService, "audit-packs");
