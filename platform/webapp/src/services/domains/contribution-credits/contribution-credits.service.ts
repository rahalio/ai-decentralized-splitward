 ...
 *
 * API client for contribution-credits domain.
...
const rawContributionCreditsService = {
  /**
   * List ContributionCredits
   */
  async getContributionCredit(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/contribution-credits` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getContributionCredit(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/contribution-credits`;
   ...
   */
  async getContributionCredit(creditId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/contribution-credits/${creditId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const contribution-creditsService = makeService(rawContributionCreditsService, "contribution-credits");
