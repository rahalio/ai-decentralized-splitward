 ...
 *
 * API client for training-rounds domain.
...
const rawTrainingRoundsService = {
  /**
   * List TrainingRounds
   */
  async getTrainingRound(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/training-rounds` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createTrainingRound(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/training-rounds`;
   ...
   * Get TrainingRound
   */
  async getTrainingRound(roundId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v0/tenants/me/training-rounds/${roundId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const training-roundsService = makeService(rawTrainingRoundsService, "training-rounds");
