/**
 * Postman-collection 1:1 Vitest tests for audit-packs (generated)
 *
...
describe("Postman / audit-packs (1:1 generated)", () => {

  it("listAuditPacks", async () => {
    const url = sub("{{baseUrl}}/v0/tenants/me/audit-packs?cursor={{cursor}}&limit={{limit}}");
  ...
  it("generateAuditPack", async () => {
    const url = sub("{{baseUrl}}/v0/tenants/me/audit-packs");
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json", ...(vars.accessToken ? { Authorization: `Bearer ${vars.accessToken}` } : {}) },
      body: sub("{\n  \"studyId\": \"newman_studyId\",\n  \"periodStart\": \"\",\n  \"periodEnd\": \"\"\n}"),
    });
  ...
  it("getAuditPack", async () => {
    const url = sub("{{baseUrl}}/v0/tenants/me/audit-packs/{{packId}}");
    const res = await fetch(url, {
      method: "GET",
      headers: vars.accessToken ? { Authorization: `Bearer ${vars.accessToken}` } : {},
    });
