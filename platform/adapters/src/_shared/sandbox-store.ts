...
export function getOrCreateTenantProfile(tenantId: string): SandboxTenantProfile {
  ...
  const profile: SandboxTenantProfile = {
    tenantId,
    displayNameEn: tenantId === 'tnt_demo' ? 'Splitward Demo Consortium' : tenantId,
    displayNameAr: tenantId === 'tnt_demo' ? 'كونسورتيوم سبليتورد التجريبي' : tenantId,
  };
...
export function generateApiKeySecret(prefix = 'splitward_demo'): string {
  return `${prefix}_${randomBytes(24).toString('hex')}`;
}


// --- Splitward product sandbox stores ---
export type JsonRecord = Record<string, unknown>;

export const sitesById = new Map<string, JsonRecord>();
export const studiesById = new Map<string, JsonRecord>();
export const trainingRoundsById = new Map<string, JsonRecord>();
export const contributionCreditsById = new Map<string, JsonRecord>();
export const incidentsById = new Map<string, JsonRecord>();
export const modelReleasesById = new Map<string, JsonRecord>();
export const localDatasetsById = new Map<string, JsonRecord>();
export const rightsExclusionsById = new Map<string, JsonRecord>();
export const auditPacksById = new Map<string, JsonRecord>();

export function listFromMap(map: Map<string, JsonRecord>): JsonRecord[] {
  return [...map.values()];
}

export function envelopeItem(item: JsonRecord, correlationId?: string) {
  return { data: item, ...responseMeta(correlationId) };
}

export function envelopeList(items: JsonRecord[], correlationId?: string) {
